...
function connect({ broker, port, username, password }, onBroadcast) {
  ...
  const url = 'mqtts://7a742d8a5acd4974b6ee7cdb4c6ca32e.s1.eu.hivemq.cloud';
  console.log(`[MQTT] Connecting to ${url} user=${username} passlen=${password?.length}`);
  ...
  client.on('error', err => {
    console.error('[MQTT] Error FULL:', JSON.stringify(err), err.message, err.code);
